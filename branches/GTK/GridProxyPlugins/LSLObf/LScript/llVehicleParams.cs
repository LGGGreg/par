...
namespace LSLObf
{
	public partial class LSLLexer
	{
		...
		const uint VEHICLE_FLAG_NO_DEFLECTION_UP = 1 << 0;
		
		// spring-loads roll only
		const uint VEHICLE_FLAG_LIMIT_ROLL_ONLY	= 1 << 1;
		
		// hover flags
		const uint VEHICLE_FLAG_HOVER_WATER_ONLY		= 1 << 2;
		const uint VEHICLE_FLAG_HOVER_TERRAIN_ONLY	= 1 << 3;
		const uint VEHICLE_FLAG_HOVER_GLOBAL_HEIGHT 	= 1 << 4;
		const uint VEHICLE_FLAG_HOVER_UP_ONLY 		= 1 << 5;
		
		// caps world-z component of linear motor to prevent 
		// climbing up into the sky
		const uint VEHICLE_FLAG_LIMIT_MOTOR_UP		= 1 << 6;
		
		const uint VEHICLE_FLAG_MOUSELOOK_STEER 		= 1 << 7;
		const uint VEHICLE_FLAG_MOUSELOOK_BANK 		= 1 << 8;
		const uint VEHICLE_FLAG_CAMERA_DECOUPLED 	= 1 << 9;
